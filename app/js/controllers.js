...
function MyCtrl1($scope,$http) {
    ...
    $scope.submit = function(){
        ...
        $http.get('http://localhost:3000/api/registerToGame/' + $scope.email +'/'+ $scope.gameType + '/' + $scope.name).
            success(function(data,status,headers,config){
                ...
                if (data !=''){
//                    $scope.partner = data;
                    window.location.href = "raffle.html?a=#icon cloud |Your partner is ||#countdown 3 |"+data.replace("\"","").replace("\"","");
                }
            }).
